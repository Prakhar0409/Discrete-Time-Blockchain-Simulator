...
public class Simulator{
	public static void main(String[] args){
		...
		for(int i=0; i<numPeers; i++){
			...
			printTree(root, tempBlockChain);
			System.out.println("\n\n");
		}
	}

	public static void printTree(String root, HashMap<String, Block> blockChain){		
		...
		if(rootBlock != null){
			...
			while(i<childListSize){
				...
				printTree(newRoot, blockChain);
				i++;
			}
			Block parent = blockChain.get(root).getParentBlock();
			if(parent != null){
				System.out.println(root+","+parent.getBlockID());
			}					
		}
	}
}